...
def make_experiment(
    ...
    """
    Signal DVC to create a new experiment using `exp save`.
    ...
    Raises:
        ExperimentExistsError: If an experiment with `name` already exists and
            `force=False`.
        UnchangedExperimentError: If there are not changes to save in the
            workspace and none of `include_untracked` or `force` has been used.

    .. _Git revision:
        https://git-scm.com/docs/revisions
    """
    with Repo.open(None) as _repo:
        exp_rev = _repo.experiments.save(
